...
export class SignalingServer {
  ...
  private nodeManager: SignalNodeManager;
  ...
  private async registerToBootstrap(): Promise<void> {
  ...
  public async handleRegister(data: RegisterMessage): Promise<{ success: boolean; nodeId?: string; nodes?: SignalNode[] }> {
  ...
  public handleHeartbeat(data: HeartbeatMessage): boolean {
  ...
  public async syncNodeList(): Promise<void> {
    const onlineNodes = this.nodeManager.getNodes('online');
    const nodeListMessage: NodeListMessage = {
      type: 'node-list',
      nodes: onlineNodes,
      timestamp: Date.now()
    };
    ...
    for (const node of onlineNodes) {
      if (node.address === this.serverAddress) continue;
  ...
  public handleNodeListSync(data: NodeListMessage): void {
  ...
  private setupSocketHandlers(): void {
    this.io.on('connection', (socket: TypedSocket) => {
      ...
      socket.on('signal:offer', (data: SignalOfferData) => {
        this.handleSignalOffer(socket, data);
      });

      socket.on('signal:answer', (data: SignalAnswerData) => {
        this.handleSignalAnswer(socket, data);
      });

      socket.on('signal:ice-candidate', (data: SignalIceCandidateData) => {
        this.handleIceCandidate(socket, data);
      });

      socket.on('signal:register', (data: RegisterMessage, callback) => {
      ...
      socket.on('signal:heartbeat', (data: HeartbeatMessage, callback) => {
  ...
  private handlePlayerJoin(socket: TypedSocket, data: PlayerJoinData): void {
    const player: Player = {
      id: uuidv4(),
      socketId: socket.id,
      name: data.name
  ...
  private handleCreateRoom(socket: TypedSocket, data: CreateRoomData): void {
  ...
  private handleListRooms(socket: TypedSocket): void {
    const roomList = Array.from(this.rooms.values()).map(room => this.getRoomInfo(room));
    socket.emit('room:list', { rooms: roomList });
  }

  private handleSignalOffer(socket: TypedSocket, data: SignalOfferData): void {
    const player = this.players.get(socket.id);
    if (!player) return;
  ...
  private handleSignalAnswer(socket: TypedSocket, data: SignalAnswerData): void {
    const player = this.players.get(socket.id);
    if (!player) return;
  ...
  private handleIceCandidate(socket: TypedSocket, data: SignalIceCandidateData): void {
  ...
  public getNodeManager(): SignalNodeManager {
